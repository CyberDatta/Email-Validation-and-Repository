...
const take_admin_input = async () => {
    try {
        ...
        const sample = await List.find({ name: "unverified" });
        console.log(
            `Syntax:
            create: C <domain>
            update: U <domain>
        ...
        while (true) {
            ...
            if (admin_cmd[0] == "C") {
                createdomainList(admin_cmd[1]);
            }
            else if (admin_cmd[0] == "U") {
                if(admin_cmd[1] == "*"){
                    var keywords_to_add=admin_cmd[2].split(",");
                ...
                deletedomainList(admin_cmd[1]);
            }
            else if (admin_cmd[0] == "R") {
                getdomainList(admin_cmd[1]);
            }
            else if (admin_cmd[0] == "E") {
